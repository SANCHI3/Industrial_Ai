...
def run(camera_id: str, source_override: str | None, display: bool, max_seconds: float | None):
    ...
    fire_smoke_detector = (
        FireSmokeDetector(camera_id, fire_smoke_cfg)
        if fire_smoke_cfg.get("enabled", False)
        else None
    )
        ...
        while True:
            ...
            for event in confirmed_events:
                ...
                if event.event_type == "fight" and event.tier == "PROBABLE":
                    ...
                    continue
                if (
                    event.event_type == "fire_smoke"
                    and event.tier == "CONFIRMED"
                    and event.previous_tier == "PROBABLE"
                    and active_fire_smoke_incident_id is not None
                ):
                    ...
                    print(
                        f"[{camera_id}] CONFIRMED fire_smoke escalation "
